...
TWITCH_CHANNEL = 'big_kage'
# if stream on YT, set to false
...
if STREAMING_ON_TWITCH is True:
...
def handle_message(message):
    try:
        ...
        ####################################
        # Move left, 2 secs
        if msg == "left" or msg == "l" or msg == "LEFT" or msg == "L" or msg == "Left":
            HoldAndReleaseKey(A, 2)

        # If the chat message is "right", then hold down the D key for 2 seconds
        if msg == "right" or msg == "r" or msg == "RIGHT" or msg == "r" or msg == "Right":
            HoldAndReleaseKey(D, 2)

        # Move up, 2 secs
        if msg == "up" or msg == "Up" or msg == "UP" or msg == "u" or msg == "U":
            HoldAndReleaseKey(W, 2)
        # Move down, 2 secs
        if msg == "down":
            HoldAndReleaseKey(S, 2)

        # Say A in chat to press E for 1 sec, can/might change. Map keyboard E to A button on controller
        if msg == "A" or msg == "a":
            HoldAndReleaseKey(E, 1)

        # Say B in chat to press R for 1 sec, can/might change
        if msg == "B" or msg == "b":
            HoldAndReleaseKey(R, 1)

        # Start
        if msg == "Start" or msg == "start" or msg == "START":
            HoldAndReleaseKey(Z, 1)

        # Select
        if msg == "Select" or msg == "select" or msg == "SELECT":
...
while True:
    ...
    if not message_queue:
        ...
        if n > 0:
            # pop message off front of queue
